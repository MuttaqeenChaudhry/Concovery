...
<template>
  ...
  props.cards.length < 4 ? `grid-cols-${props.cards.length}` : 'grid-cols-4',
      ...
      <div v-for="(item, index) in props.cards" :key="index" ref="card" class="card block rounded border border-transparent px-3 py-5 transition-all duration-200 cursor-pointer overflow-hidden" :style="{
  zIndex: index + 1,
}">
        <b name="logo" :logo="item.logo" :index="index">{{ item }}</b>
      </div>
    </div>
  </div>
</template>
